...
export default function App() {
  // --- Loading State ---
  // Added 'READY' state to handle the "Adventure Can Begin" click interaction
  const [bootPhase, setBootPhase] = useState<'LOADING' | 'READY' | 'ZOOMING' | 'MENU'>('LOADING');
  ...
  useEffect(() => {
    ...
    const bootGame = async () => {
        ...
        // Wait small delay then show Ready button instead of zooming immediately
        setTimeout(() => {
            setBootPhase('READY');
        }, 500);
    };
  ...
  const handleStartAdventure = () => {
      unlockAudio(); // Triggers music thanks to user interaction
  ...
  // NOW INCLUDES 'READY' STATE
  if (bootPhase === 'LOADING' || bootPhase === 'READY' || bootPhase === 'ZOOMING') {
      return (
              ...
              <div className="relative z-50 w-full flex flex-col items-center px-8 md:px-32 lg:px-64 gap-6 animate-in fade-in slide-in-from-bottom-5 duration-1000">
                  ...
                  {bootPhase === 'READY' ? (
                       <button 
                           ...
                           className="group bg-[#1a120b]/80 hover:bg-[#2c1e16] text-[#e6c888] font-serif font-bold text-xl md:text-3xl py-6 px-12 md:px-20 border-2 border-[#8a6e38] hover:border-[#c5a059] uppercase tracking-[0.2em] shadow-[0_0_40px_rgba(197,160,89,0.4)] animate-pulse hover:scale-105 transition-all duration-300 backdrop-blur-md"
                       >
                           <span className="group-hover:text-[#ffd700] transition-colors drop-shadow-lg">Your Adventure Can Begin</span>
                       </button>
                  ) : (
                      <>
                          {/* Tip / Lore Container */}
